...
// end constructor
    ...
    function query($the_query) {
        ...
        $tbl = $parser['TBL'];
        $check = $this->db->query("SELECT 1 FROM $tbl");
        if(!empty($check)){
            ...
            $this->query_id->execute() or die($this->db->error);
            $end  = microtime();
...
// end select
    ...
    function do_update($tbl, $arr, $where = "", $is_test = 0) {
        ...
        if(!empty($check)){
            ...
            $ci = $this->db->prepare($parser['QUERY']) or die($this->db->error);
    ...
    function do_insert($tbl, $arr, $is_test=0) {        
        ...
        if(!empty($check)){
            ...
            $ci = $this->db->prepare($sql_prepare) or die($this->db->error);
            $arr_value = array_values($arr);
