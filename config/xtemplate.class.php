...
class XTemplate {
	...
	/**
	 * Template tag start script
	 *
	 * This makes the delimiters look like:
	 * @example <script>content</script>
	 * if you use the default syntax.
	 *
	 * @access public
	 * @var string
	 */
	public $tag_start_script = '<script>';

	/**
	 * Template tag end script
	 *
	 * This makes the delimiters look like:
	 * @example <script>content</script>
	 * if you use the default syntax.
	 ...
	 * @var string
	 */
	public $tag_end_script = '</script>';
	/* this makes the delimiters look like: <script>content</script> if you use my syntax. */
	...
	public function parse ($bname) {
        ...
        if(!($this->filename === $ims->path_html.'html.tpl' && $bname === 'body')
           && !($this->filename === $ims->path_html.'html.tpl' && $bname === 'popup')) {
            preg_match_all('{<script[^>]*>((?:(?:(?!<script[^>]*>|</script>).)++|<script[^>]*>(?1)</script>)*)</script>}si', $copy, $var_array_script);  

            if(count($var_array_script[0])) {
                foreach($var_array_script[0] as $tk => $tv) {

                    if(isset($var_array_script[1][$tk]) && $var_array_script[1][$tk]) {
                        $copy = str_replace($tv, '', $copy);                
                        $ims->func->include_js_content($var_array_script[1][$tk]);
                    }
                }
            }
        }
