...
$(document).ready(function(){
		...
		var options = {
		  callback: function(key, options) {
			...
			case "unzip":
			    ...
			    $.ajax({
				type: "POST",
				url: "ajax.php?m=library&sub=ajax_calls&folder_up="+folder_up+"&action=extract",
				data: { path: m }
			    }).done(function( msg ) {
				if (msg!="")
				    bootbox.alert(msg);
	...
	if (js_script) {
	    $.ajax({
		url: "ajax.php?m=library&sub=ajax_calls&action=sort&sort_by="+_this.attr('data-sort')+"&descending="+sortDescending
	    }).done(function( msg ) {
	...
	bootbox.prompt($('#insert_folder_name').val(),$('#cancel').val(),$('#ok').val(), function(name) {
		...
		$.ajax({
			  type: "POST",
			  url: "ajax.php?m=library&sub=execute&folder_up="+folder_up+"&action=create_folder",
    ...
    $('.view-controller button').on('click',function(){
	    ...
	    _this.find('i').addClass('icon-white');
	     ...
	     $.ajax({
		url: "ajax.php?m=library&sub=ajax_calls&action=view&type="+_this.attr('data-value')
	    }).done(function( msg ) {
		if (msg!="") {
		    bootbox.alert(msg);
		}   
...
function apply_muti(file,external){
    ...
    parent.tinymce.activeEditor.insertContent(fill);
		$('#ims-mess').html('<div class="ims-mess mess-success">Đã thêm file <strong>'+file+'</strong> vào trình soạn thảo</div>');
    //parent.tinymce.activeEditor.windowManager.close();
}
...
function apply_img(file,external){
    ...
    if (external!=""){
	var target = $('#'+external, window_parent.document);
	var review = $('#show_'+external, window_parent.document);
	$(review).html('<div class="item"><img src="'+base_url+path+file+'"></div>');
	$(target).val(base_url+path+file);
	$(target).trigger( "change" );
	if($(target).data('event') == 'click') {
		$(target).click();
	}
	close_window();
    }
...
function apply_img_review(file,external){
    if ($('#popup').val()==1) var window_parent=window.opener; else var window_parent=window.parent;
    var path = $('#cur_dir').val();
    path = path.replace('\\', '/');
    var base_url = $('#base_url').val();
...
function apply_img_muti(file,external){
    if ($('#popup').val()==1) var window_parent=window.opener; else var window_parent=window.parent;
    var path = $('#cur_dir').val();
    path = path.replace('\\', '/');
    var base_url = $('#base_url').val();
    ...
    if (external!=""){
			var target = $('#'+external, window_parent.document);
			var html_name = ($(target).data("html_name")) ? $(target).data("html_name") : 'arr_picture[]';
			//$(target).val(base_url+path+file);
			//$(target).trigger( "change" );
			
			$(target).append('<div class="pic-item"><a class="fancybox-effects-a" href="'+base_url+path+file+'"><img src="'+base_url+path+file+'" alt="'+file+'" /></a><a href="javascript:;" class="btn-remove"><i class="fal fa-times-circle"></i></a><input name="'+html_name+'" value="'+base_url+path+file+'" type="hidden" /></div>');
			$('#ims-mess').html('<div class="ims-mess mess-success">Đã thêm file <strong>'+file+'</strong> thành công!</div>');
...
function execute_action(action,file1,file2,name,container,function_name){
	...
	$.ajax({
	    type: "POST",
	    url: "ajax.php?m=library&sub=execute&folder_up="+folder_up+"&action="+action,
