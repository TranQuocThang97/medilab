...
if (isset($ims->get['fldr'])
		&& !empty($ims->get['fldr'])
		&& strpos($ims->get['fldr'],'../')===FALSE
		&& strpos($ims->get['fldr'],'./')===FALSE)
		$subdir = urldecode(trim($ims->get['fldr'],"/") ."/");
else
		$subdir = '';
...
if(isset($ims->get['popup'])) $popup= $ims->get['popup']; else $popup=0;

//view type
if(!isset($_SESSION["view_type"])){ $view=$default_view; $_SESSION["view_type"] = $view; }
if(isset($ims->get['view'])){ $view=$ims->get['view']; $_SESSION["view_type"] = $view; }
$view=$_SESSION["view_type"];

if(isset($ims->get["filter"])) $filter=fix_filename($ims->get["filter"],$transliteration);
else $filter='';

if(!isset($_SESSION['sort_by'])) $_SESSION['sort_by']='';
if(isset($ims->get["sort_by"])) $sort_by=$_SESSION['sort_by']=fix_filename($ims->get["sort_by"],$transliteration);
else $sort_by=$_SESSION['sort_by'];

if(!isset($_SESSION['descending'])) $_SESSION['descending']=false;
if(isset($ims->get["descending"])) $descending=$_SESSION['descending']=fix_filename($ims->get["descending"],$transliteration)==="true";
...
if(!isset($ims->get['type'])) $ims->get['type']=0;
if(!isset($ims->get['field_id'])) $ims->get['field_id']='';
$get_params = http_build_query(array(
		'type'      => $ims->get['type'],
		'lang'      => $lang,
		'popup'     => $popup,
		'field_id'  => isset($ims->get['field_id']) ? $ims->get['field_id'] : '',
		'folder_up'     => $ims->conf["folder_up"],
		'fldr'      => ''
));
?>
...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<link rel="shortcut icon" href="<?php echo $ims->dir_mod?>img/ico/favicon.ico">
	<link href="<?php echo $ims->dir_mod?>css/bootstrap.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ims->dir_mod?>css/bootstrap-responsive.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ims->dir_mod?>css/bootstrap-lightbox.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ims->dir_mod?>css/style.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ims->dir_mod?>css/dropzone.min.css" type="text/css" rel="stylesheet" />
	<link href="<?php echo $ims->dir_mod?>css/jquery.contextMenu.min.css" rel="stylesheet" type="text/css" />	
	<link href="<?php echo $ims->dir_mod?>css/bootstrap-modal.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ims->dir_mod?>jPlayer/skin/blue.monday/jplayer.blue.monday.css" rel="stylesheet" type="text/css">
	<!--[if lt IE 8]><style>
	...
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/jquery.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/bootstrap.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/bootstrap-lightbox.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/dropzone.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/jquery.touchSwipe.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/modernizr.custom.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/bootbox.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/bootstrap-modal.min.js"></script>   
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/bootstrap-modalmanager.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>jPlayer/jquery.jplayer.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/imagesloaded.pkgd.min.js"></script>
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/jquery.queryloader2.min.js"></script>
	
	<!-- Le HTML5 shim, for IE6-8 support of HTML5 elements -->
	<!--[if lt IE 9]>
		<script src="<?php echo $ims->dir_mod?>js/html5shiv.js"></script>
	<![endif]-->
	<script src="<?php echo $ims->dir_mod?>js/jquery.ui.position.min.js" type="text/javascript"></script>
	<script src="<?php echo $ims->dir_mod?>js/jquery.contextMenu.min.js" type="text/javascript"></script>    
	
	<script>
			var folder_up = "<?php echo $ims->conf["folder_up"]; ?>";
			...
			Dropzone.options.myAwesomeDropzone = {
				...
				url: "<?php echo $this->link_act?>&sub=upload&folder_up=<?php echo $ims->conf["folder_up"]?>",
	...
	<script type="text/javascript" src="<?php echo $ims->dir_mod?>js/include.js"></script>
		</head>
...
<div class="uploader">
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
		...
		<form action="<?php echo $this->link_act?>&sub=popup_library" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
			...
			<input type="hidden" name="type" value="<?php echo $ims->get['type']; ?>"/>
			<input type="hidden" name="field_id" value="<?php echo $ims->get['field_id']; ?>"/>
...
$class_ext = '';
$src = '';
if ($ims->get['type']==1) 	 $apply = 'apply_img';
elseif($ims->get['type']==2) $apply = 'apply_link';
elseif($ims->get['type']==0 && $ims->get['field_id']=='') $apply = 'apply_none';
elseif($ims->get['type']==3) $apply = 'apply_video';
elseif($ims->get['type']==4) $apply = 'apply_muti';
elseif($ims->get['type']==5) $apply = 'apply_img_muti';
else $apply = 'apply';
...
<div class="navbar navbar-fixed-top">
		...
		<div class="filters">
			...
			<div class="span4 half">
					...
					<?php if($upload_files){ ?>
								<button class="tip btn upload-btn" title="<?php echo  lang_Upload_file; ?>"><i class="icon-plus"></i><i class="icon-file"></i>  Tải hình lên</button> 
					<?php } ?>
			...
			<div class="span5 types">
					<span><?php echo lang_Filters; ?>:</span>
					<?php if($ims->get['type']!=1 && $ims->get['type']!=3){ ?>
					...
					<input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
					 <label id="ff-item-type-all" title="<?php echo lang_All; ?>" <?php if($ims->get['type']==1 || $ims->get['type']==3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
...
<!----- header div end ------->
	<div id="ims-mess" class="row-fluid"></div>
		...
		foreach ($files as $file_array) {
			...
			<li data-name="<?php echo $file ?>" <?php if($file=='..') echo 'class="back"'; else echo 'class="dir"'; ?>>
				<figure data-name="<?php echo $file ?>" class="<?php if($file=="..") echo "back-"; ?>directory" data-type="<?php if($file!=".."){ echo "dir"; } ?>">
					...
					<img class="directory-img"  src="<?php echo $ims->dir_mod?>img/<?php echo $icon_theme; ?>/folder<?php if($file==".."){ echo "_back"; }?>.jpg" alt="folder" />
					</div>
						</div>
						<div class="img-precontainer-mini directory">
					<div class="img-container-mini">
							<span></span>
							<img class="directory-img"  src="<?php echo $ims->dir_mod?>img/<?php echo $icon_theme; ?>/folder<?php if($file==".."){ echo "_back"; }?>.png" alt="folder" />
					</div>
						</div>
			<?php if($file==".."){ ?>
			...
			$file=$file_array['file'];
				...
				$mini_src = $thumbs_base_src.$subdir. $file;
				//add in thumbs folder if not exist
				...
				if($img_width<45 && $img_height<38){
					...
					if($src_thumb==""){
				if(file_exists($ims->dir_mod.'img/'.$icon_theme.'/'.$extension_lower.".jpg")){
					$src_thumb =$ims->dir_mod.'img/'.$icon_theme.'/'.$extension_lower.".jpg";
				}else{
					$src_thumb = $ims->dir_mod."img/".$icon_theme."/default.jpg";
				}
				$is_icon_thumb=true;
					}
					if($mini_src==""){
				$is_icon_thumb_mini=false;
					...
					if((!($ims->get['type']==1 && !$is_img) && !(($ims->get['type']==3 && !$is_video) && ($ims->get['type']==3 && !$is_audio))) && $class_ext>0){
?>
		<li class="ff-item-type-<?php echo $class_ext; ?> file"  data-name="<?php echo $file; ?>">
			<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				<a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $ims->get['field_id']; ?>" data-function="<?php echo $apply; ?>">
				<div class="img-precontainer">
				...
				<div class="box">				
				<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $ims->get['field_id']; ?>" data-function="<?php echo $apply; ?>">
				...
				<figcaption>
						<form action="<?php echo $ims->dir_mod?>force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
	...
	?></div>
		...
		<div id="loading_container" style="display:none;">
			...
			<img id="loading_animation" src="<?php echo $ims->dir_mod?>img/storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%"/>
		</div>
		<!----- loading div end ------->
