...
function html_list_group ($arr_in = array())
{
	...
	$link_action = (isset($arr_in['link_action'])) ? $arr_in['link_action'] : $ims->site_func->get_link ('recruitment');
	$temp = (isset($arr_in['temp'])) ? $arr_in['temp'] : 'list_item';
	$p = (isset($ims->input["p"])) ? $ims->input["p"] : 1;
	$pic_w = 164;//(isset($ims->setting['recruitment']["img_list_w"])) ? $ims->setting['recruitment']["img_list_w"] : 100;
	$pic_h = 164;//(isset($ims->setting['recruitment']["img_list_h"])) ? $ims->setting['recruitment']["img_list_h"] : 100;
	$num_row = 1;
	...
	$n = (isset($ims->setting['recruitment']["num_list"])) ? $ims->setting['recruitment']["num_list"] : 30;
	...
	if ($num = $ims->db->num_rows($result))
	{
		...
		while ($row = $ims->db->fetch_row($result)) 
		{
			...
			$row['pic_w'] = $pic_w;
			$row['pic_h'] = $pic_h;
			$row['link'] = $ims->site_func->get_link ('recruitment',$row['friendly_link']);
	...
	{
		$ims->temp_act->assign('row', array("mess"=>$ims->lang["recruitment"]["no_have_item"]));
...
function html_list_item ($arr_in = array())
{
	...
	$keyword = isset($ims->get['keyword']) ? '&keyword='.$ims->get['keyword'] : '';
	$type = isset($ims->get['type']) ? '&type='.$ims->get['type'] : '';	
    $province = isset($ims->get['province']) ? '&province='.$ims->get['province'] : '';
    $ext = $keyword.$type.$province;	
	$where = (isset($arr_in['where'])) ? $arr_in['where'] : '';
	...
	$sql = "select item_id,group_id,picture,picture_icon,title,content,quantity,province,friendly_link,date_end,date_update  
					from recruitment 
					where is_show=1 
					and lang='".$ims->conf["lang_cur"]."' 
					".$where." 
	...
	if ($num = $ims->db->num_rows($result))
	{
		...
		while ($row = $ims->db->fetch_row($result)) 
		{
			...
			$row['link'] = $ims->site_func->get_link ('recruitment','',$row['friendly_link']);
			if($row["picture_icon"]!=''){
				$row["picture"] = $ims->func->get_src_mod($row["picture_icon"], $pic_w, $pic_h, 1, 0, array('fix_max' => '1'));	
			}else{
				$row['picture'] = $ims->dir_images."resume_icon.png";
			}        	        	
			$row['short'] = $ims->func->short ($row['content'], $short_len);
			$row['date_update'] = date('d/m/Y',$row['date_update']);
			if($row['province'] != 0){
				$row['location'] = $ims->db->load_item('location_province','is_show=1 and code="'.$row['province'].'"','title');
			}			
			if($row['quantity'] != ''){
				$row['quantity'] = $ims->func->input_editor_decode($row['quantity']);
			}
			// $row['class'] = ($i%$num_row == 0 || $i == $num) ? ' last' : '';
