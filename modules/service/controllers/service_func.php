...
function html_list_group ($arr_in = array())
{
	...
	$link_action = (isset($arr_in['link_action'])) ? $arr_in['link_action'] : $ims->site->get_link ('service');
	$temp = (isset($arr_in['temp'])) ? $arr_in['temp'] : 'list_item';
	$p = (isset($ims->input["p"])) ? $ims->input["p"] : 1;
	$pic_w = 164;//(isset($ims->setting['service']["img_list_w"])) ? $ims->setting['service']["img_list_w"] : 100;
	$pic_h = 164;//(isset($ims->setting['service']["img_list_h"])) ? $ims->setting['service']["img_list_h"] : 100;
	$num_row = 1;
	...
	$n = (isset($ims->setting['service']["num_list"])) ? $ims->setting['service']["num_list"] : 30;
	...
	if ($num = $ims->db->num_rows($result))
	{
		...
		while ($row = $ims->db->fetch_row($result)) 
		{
			...
			$row['pic_w'] = $pic_w;
			$row['pic_h'] = $pic_h;
			$row['link'] = $ims->site->get_link ('service',$row['friendly_link']);
	...
	{
		$ims->temp_act->assign('row', array("mess"=>$ims->lang["service"]["no_have_item"]));
...
function box_menu () {
	...
	if(!isset($ims->data["service"])){
		...
		$ims->data["service"] = array();
		if($num = $ims->db->num_rows($result)){
			while($row = $ims->db->fetch_row($result)){
				$ims->data["service"][$row["item_id"]] = $row;
			}
		}
	}
	...
	if(count($ims->data["service"]) > 0){
		$data = array(
			'title' => $ims->lang['service']['menu_title'],
			'content' => ''
		);
		
		$menu_sub = '';
		$i = 0;
		foreach($ims->data["service"] as $row)
		{
			$i++;
			$row['link'] = $ims->site->get_link ('service','',$row['friendly_link']);
