...
class sMain {
    ...
    function __construct() {
        ...
        if (isset($ims->conf['cur_item']) && isset($ims->data['cur_item']) && $ims->data['cur_item']) {
            ...
            $ims->db->query("UPDATE support set num_view=num_view+1 WHERE item_id='".$ims->data['cur_item']['item_id']."'");

            $row = $ims->data['cur_item'];
            //Make link lang
            $result = $ims->db->query("SELECT friendly_link,lang FROM support WHERE item_id='".$ims->conf['cur_item']."' ");
...
//        $data["link_share"]  = $ims->site_func->get_link('support', '', $info['friendly_link']);
    ...
    function do_other($info) {
        ...
        $arr_in = array(
            ...
            'num_list' => $ims->setting['support']["num_order_detail"],
            'pic_w' => 291,
            'pic_h' => 123,
            'paginate' => 0,
        );
        ...
        $check = $ims->db->load_item('support', $ims->conf['qr'].$arr_in['where'], 'item_id');
        if($check){
    ...
    function do_most_read(){
        global $ims;
        $limit = ($ims->setting['support']['num_order_detail']) ? $ims->setting['support']['num_order_detail'] : 8;
        $result = $ims->db->load_item_arr('support', $ims->conf['qr'].' order by num_view desc, date_create desc limit '.$limit, 'title, picture, friendly_link');
        if($result){
