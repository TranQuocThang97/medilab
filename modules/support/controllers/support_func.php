...
class supportFunc {
    ...
    public $modules     = "support";
    public $parent      = null;
    public $parent_mod  = "support";
    public $parent_act  = "support";
    public $temp_act    = "";
    ...
    // where_support
    function where_support($type = 'support') {
        global $ims;
    ...
    // support_loaded
    function support_loaded($id = 0, $type = 'support') {
        global $ims;
    ...
    function get_group_name($group_id, $type = 'none') {
        ...
        if ($row = $ims->db->fetch_row($result)) {
            switch ($type) {
                case "link":
                    $link = $ims->site_func->get_link('support', $row['friendly_link']);
...
//        }
        ...
        $this->support_loaded($row['item_id'], $this->modules);
    ...
    function html_list_item($arr_in = array(), $type = '') {
        ...
        $link_action = (isset($arr_in['link_action'])) ? $arr_in['link_action'] : $ims->site_func->get_link('support');
        ...
        $n = (isset($ims->setting['support']["num_list"])) ? $ims->setting['support']["num_list"] : 30;
        ...
        $where .= $this->where_support();

        $nav = '';
        $num_total = 0;
        $start = 0;
        if ($paginate == 1) {
            $num_total = $ims->db->do_get_num("support", "1 ". $ims->conf['where_lang'] . $where);
        ...
        $result = $ims->db->load_item_arr('support', $ims->conf['qr'].$where.' limit '.$start.','.$n, 'item_id, title, picture, friendly_link, short, content, date_create, group_id');
        if ($result) {
            $i = 0;
            ...
            $this->temp_func->assign('row', array("mess" => $ims->lang["support"]["no_have_item"]));
...
//=================select===============
    function box_menu_sub($array = array()) {
        ...
        foreach ($array as $row) {
            $row['link'] = $ims->site_func->get_link('support', $row['friendly_link']);
    ...
    function box_menu() {
        ...
        if (!isset($ims->data["support_group"])) {
            ...
            $ims->data["support_group"] = array();
            $ims->data["support_group_tree"] = array();
            if ($num = $ims->db->num_rows($result)) {
                while ($row = $ims->db->fetch_row($result)) {
                    $ims->data["support_group"][$row["group_id"]] = $row;
                    ...
                    eval('$ims->data["support_group_tree"]' . $str_code . '["group_id"] = $row["group_id"];
				$ims->data["support_group_tree"]' . $str_code . '["title"] = $row["title"];
				$ims->data["support_group_tree"]' . $str_code . '["friendly_link"] = $row["friendly_link"];');
                }
            }
        }
        ...
        if (count($ims->data["support_group_tree"]) > 0) {
            $data = array(
                'title' => $ims->lang['support']['menu_title'],
                'content' => ''
            );

            $menu_sub = '';
            foreach ($ims->data["support_group_tree"] as $row) {
                $row['link'] = $ims->site_func->get_link('support', $row['friendly_link']);
                $row['class'] = (in_array($row["group_id"], $arr_cur)) ? ' class="current"' : '';
                $row['menu_sub'] = '';
                if (isset($row['arr_sub'])) {
    ...
    function get_navigation() {
        ...
        $arr_nav = array(
            ...
            array(
                'title' => $ims->setting['support']['support_meta_title'],
                'link' => $ims->site_func->get_link('support')
            )
        );
        ...
        foreach ($arr_group as $group_id) {
            if (isset($ims->data["support_group"][$group_id])) {
                $arr_nav[] = array(
                    'title' => $ims->data["support_group"][$group_id]['title'],
                    'link' => $ims->site_func->get_link('support', $ims->data["support_group"][$group_id]['friendly_link'])
                );
            }
        }
        ...
        if (isset($ims->conf['cur_item']) && $ims->conf['cur_item'] > 0 && isset($ims->data["cur_item"]['friendly_link'])) {
            $arr_nav[] = array(
                ...
                'link' => $ims->site_func->get_link('support', '', $ims->data["cur_item"]['friendly_link'])
            );
        }
    ...
    function list_item_related($arr_in = array()) {
        ...
        $n = (isset($ims->setting['support']["num_order_detail"])) ? $ims->setting['support']["num_order_detail"] : 30;
        $n = (isset($arr_in["num_show"])) ? $arr_in["num_show"] : $n;
        $title = (isset($arr_in["title"])) ? $arr_in["title"] : $ims->lang['support']['other_support'];
    ...
    function list_other($arr_in = array()) {
        ...
        $n = (isset($ims->setting['support']["num_order_detail"])) ? $ims->setting['support']["num_order_detail"] : 5;
        $n = (isset($arr_in["num_show"])) ? $arr_in["num_show"] : $n;
        $title = (isset($arr_in["title"])) ? $arr_in["title"] : $ims->lang['support']['other_support'];
