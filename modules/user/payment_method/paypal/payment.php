...
class Payment 
{
	...
	{
		global $ims;
		require_once($ims->conf['rootpath'].'modules'.DS.'product'.DS.'payment_method'.DS.'config'.DS.'paypal.php' ); //include config
		...
		, $currencyCodeType='SGD')
	{
		...
		$params = array (
			...
			'LOGOIMG' => $ims->conf['rooturl'].'logo-on-paypal.png',
		...
		if ($order_info = $ims->db->fetch_row($result)) {
