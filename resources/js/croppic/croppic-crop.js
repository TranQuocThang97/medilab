...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		bindImgUploadControl: function(){
			...
			var formHtml = '<input id="'+that.id+'_imgInput" type="hidden" value="" readonly="readonly" data-event="click">';
			...
			//Viet them
			$('#'+that.id+'_imgInput').click(function() {
				var tmpImg = new Image();
				tmpImg.src=$(this).val(); //or  document.images[i].src;
				$(tmpImg).on("load",function(){
					that.imgUploadControl.hide();
					that.imgInitW = that.imgW = tmpImg.width;
					that.imgInitH = that.imgH = tmpImg.height;
					
					if(that.options.modal){	that.createModal(); }
					if( !$.isEmptyObject(that.croppedImg)){ that.croppedImg.remove(); }
					
					that.imgUrl=tmpImg.src;
		...
		createModal: function(){
			...
			that.objH = that.objH * that.options.modalRatio;
			that.objW = that.objW * that.options.modalRatio;
