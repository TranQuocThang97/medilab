	...
	*/
	...
	function updateCart(){
		...
		if ($ims->method == 'POST'){			
			...
			// $product = $ims->db->load_item('product','item_id="'.$item_id.'" AND lang="'.$ims->conf['lang_cur'].'" AND is_show=1','id');
			// if (empty($product)) {
			// 	$array = array(
			// 		"code" => 400,
   //  				"message" => $ims->lang['api']['error_getRatingProduct_2'],
   //          	);
			// 	$this->response(400, $array);
			// }
			// $where = '';
			// if(!empty($combo_info)){
			// 	$where .= ' AND combo_info="'.$combo_info.'" ';
			// }//.$where.''
	            ...
	            if(!empty($ims->setting['product']['is_freeship'])){
	                ...
	                if(!empty($arr_price) && is_array($arr_price)){
	                    ...
	                    foreach($arr_price as $key => $value){	                    	
	                        $radius = $value['radius'];
	                        if($value['province']==$province && $input['totalProvisional']>=$ims->setting['product']['ototal_freeship']){	                        	
	                    ...
	                    if(!empty(min($tmp)) && min($tmp)<=$radius){
	                        $array = array(
								"code" => 200,
					    		"message" => $ims->lang['api']['success'],
								"data" => 0
	...
	function orderComplete(){
		...
		if ($ims->method == 'POST'){
			...
			if ($ok) {
				...
				$col_up["promotion_price"] 		  = $promotion_price;
